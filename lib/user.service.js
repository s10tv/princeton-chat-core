...
export default class UserService {
  ...
  static getUserView (user) {
    ...
    if (user.firstName) {
      user._displayName += `${user.firstName} `
    ...
    if (user.lastName) {
      ...
      user._displayName += `${user.lastName} `
    }

    if (user.classDisplay) {
      user._displayName += `${user.classDisplay}`
    }

    // if the user does not come with a displayName stored, use our generated one
    user.displayName = user.displayName || user._displayName
